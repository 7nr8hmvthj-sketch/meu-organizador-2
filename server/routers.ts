...
const normalizeEvent = (event: any) => ({
  ...
  workplaceId: event.workplaceid ?? event.workplaceId ?? null,
...
export const appRouter = router({
  ...
  events: router({
    ...
    update: protectedProcedure
      .input(z.object({ id: z.number(), date: z.string().optional(), type: z.string().optional(), description: z.string().optional(), startTime: z.string().optional(), endTime: z.string().optional(), color: z.string().optional(), isPassed: z.boolean().optional(), passedReason: z.string().optional(), workplaceId: z.number().nullable().optional() }))
      .mutation(async ({ input, ctx }) => {
    ...
    monthlySummary: adminProcedure
      ...
      .query(async ({ ctx, input }) => {
        ...
        for (const wp of userWorkplaces) {
          ...
          // LÓGICA CORRIGIDA: Sempre mostra o ciclo ANTERIOR (já fechado)
          // Se estamos em maio, mostra ciclo de março/abril
          // Se estamos em junho, mostra ciclo de abril/maio
          let cycleMonth = input.month - 1;
          let cycleYear = input.year;
          if (cycleMonth < 1) { cycleMonth = 12; cycleYear--; }
          
          // Calcula o mês de início do ciclo (pode ser no mês anterior se cycleStartDay > cycleEndDay)
          let cycleStartMonth = cycleMonth;
          let cycleStartYear = cycleYear;
          if (wp.cycleStartDay > wp.cycleEndDay) {
            cycleStartMonth = cycleMonth - 1;
          ...
          const refEnd = `${cycleYear}-${String(cycleMonth).padStart(2, '0')}-${String(wp.cycleEndDay).padStart(2, '0')}`;
